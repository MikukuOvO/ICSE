...
def set_service_cpu(namespace: str, service: str, cpu_cores: float) -> str:
    ...
    try:
        ...
        if delta > (current_cpu / 4):
            return f"Error: The change in CPU allocation ({delta:.3f} cores) is too large (must be <= {current_cpu/4:.3f} cores, which is 1/4 of the current allocation of {current_cpu:.3f} cores)"        
